...
def main():
    ...
    for user_id, similarity in similar_users:
        ...
        common_items = np.intersect1d(
            np.where(user_ratings > 0)[0],
            np.where(similar_user_ratings > 0)[0]
        )
        print(f"{user_id}\t{similarity:.4f}\t{len(common_items)}")
        
        # 显示共同评分的电影
        if len(common_items) > 0:
            print("\n共同评分的电影:")
            print("电影ID\t用户评分\t相似用户评分\t电影标题")
            print("-" * 70)
            for item_idx in common_items[:5]:  # 只显示前5个共同评分
                movie_id = list(item_idx_map.keys())[list(item_idx_map.values()).index(item_idx)]
                movie_info = movies[movies['movieId'] == movie_id].iloc[0]
                print(f"{movie_id}\t{user_ratings[item_idx]:.1f}\t{similar_user_ratings[item_idx]:.1f}\t{movie_info['title']}")
